...
浏览器**默认有头**（2026-09-21 起，此前默认无头）。屏幕上会出现窗口、会抢一下键盘焦点，
这是**正常的，不是故障**。

**❗别为了「不打扰用户」擅自改走无头。** 无头 Chrome 的 User-Agent 里带 `HeadlessChrome/<ver>`，
等于每个请求都自报「我是自动化工具」——本 CLI 刻意不覆盖 UA（伪造 UA 与 Client Hints 矛盾，
是更糟的指纹），绕不过去。BOSS 侧因此被限过 web 端登录；猎聘侧 2026-09-20 实测连续无头调用后
账号被判「行为异常」、302 到验证页、业务接口静默拦截。用户明确要求才设 `LIEPIN_HEADLESS=true`。

浏览器还**跨命令常驻**（占固定端口 53471，命令结束只断 CDP），下条命令复用同一只、同一登录态。

- **不想让窗口抢焦点** → 用 DSH 的「招聘浏览器」面板（实时画面推到 Web UI，能看能点，不抢焦点），
  而不是改无头
- **用户明确要求隐藏** → `LIEPIN_HEADLESS=true liepin <cmd>`，并告知上面的风控代价
- **换了变量没生效？** 端口上已有实例会被复用。先 `liepin quit`，下条命令才会按新模式重启
